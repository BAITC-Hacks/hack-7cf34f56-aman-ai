...
try {
  ...
  const result = await client.callTool({ name: runTool.name, arguments: { code: `async (page) => {
    ...
    await page.getByText('Проследите движение денег').waitFor();
    ...
    assert(await page.getByText('Проследите движение денег').isVisible(), 'Initial no-selection screen');
    ...
    assert(await page.locator('.react-flow__node').count() === 10, 'Local one-hop graph');
    ...
    await page.getByText('СООБЩЕСТВО В СЕТИ').waitFor();
    await page.locator('.cluster-view').getByRole('button', {name:'900000000000100001', exact:true}).click();
    await page.locator('.node-gid').filter({hasText:'900000000000100001'}).waitFor();
    passed.push('Cluster summary and return selection');
    await page.getByRole('radio', {name:'2 шага',exact:true}).click();
    assert(page.url().includes('hop=2'), 'Two-hop URL state');
    ...
    await page.getByText('Нет наблюдаемых связей', {exact:true}).waitFor();
    ...
    await page.goto('http://127.0.0.1:5173/');
    await page.getByText('Не удалось загрузить', {exact:true}).waitFor();
    fail=false;
    ...
    await page.goto('http://127.0.0.1:5173/');
    await page.getByText(/Ответ не соответствует контракту данных/).waitFor();
