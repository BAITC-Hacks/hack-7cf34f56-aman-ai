...
class RealProjectDataTests(unittest.TestCase):
    def test_existing_dataset_is_consistent_and_retains_exact_source_gids(self):
        ...
        source_top = pd.read_csv(root / "results/top_nodes.csv", dtype={"gid": str})
        self.assertEqual([node["gid"] for node in dataset["top"]], source_top["gid"].tolist())
        self.assertEqual([node["priority_score"] for node in dataset["top"]], source_top["priority_score"].tolist())
