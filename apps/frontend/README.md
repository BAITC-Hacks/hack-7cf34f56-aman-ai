...
From the repository root, **`docker compose up --build`** installs dependencies, verifies both analytical runs, builds the frontend, and serves http://localhost:8080. It needs Docker Engine and Compose v2; the first build needs network access. Stop with `Ctrl+C`, then `docker compose down`. Local Compose binds only to loopback and does not forward the root `.env` or enable AI. The three jury CSVs are retained at `/app/results` for copying to a new local directory.

The Render Blueprint deploys the same image on a Free web service, with a generated access password and username `judge`. No hosted deployment has been completed yet. Docker is unavailable in this verification environment, so the image build and Compose commands have not been executed here. See [deployment instructions and verification limits](../../docs/deployment.md) for CSV extraction, Render account/repository access, manual deployment, and runtime settings.

`npm start` serves an existing `dist/` using `server/production.mjs`; it does not load `.env`. The data mode comes from built `dist/runtime-config.json`, and a conflicting runtime `VITE_DATA_MODE` fails startup. After building, a local-only production check is `APP_ACCESS_MODE=public HOST=127.0.0.1 PORT=8080 npm start` from this directory. For hosted project data, retain the default password mode and provide `APP_ACCESS_PASSWORD` through the host environment. `/healthz` is public; the dashboard, dataset, and chat require authentication.

Optional OpenAI credentials belong in runtime environment variables only. The default global chat budget is 20 requests/hour per process (`AGENT_REQUESTS_PER_HOUR`), resets after a restart, and is not a hard spending cap. A custom hostname needs its HTTPS origin in `APP_ORIGIN`; Render's default hostname uses `RENDER_EXTERNAL_URL`. `npm run test:server` verifies the production HTTP behavior independently of Docker.
...
For local development, use the repository-root `.env.example` as a template, set `OPENAI_API_KEY` in the root `.env`, then restart the dev server. Production receives the key through the hosting environment; `npm start` does not read `.env`. The default chat model is `gpt-4.1-mini`; `OPENAI_MODEL` overrides it. The server uses the official OpenAI SDK and Responses API with tools that retrieve calculated graph facts. The key stays on the server; never give it a `VITE_` prefix. No AI request is needed for graph exploration or client inspection.

`npm run dev` and `npm run preview` include the server-side bridge at `/api/agent/status` and `/api/agent/chat`, using validated `public/project-data.json` in project mode. The production `npm start` server serves the same routes alongside the built frontend, reads the snapshot from `dist/`, and applies the configured access password. In the development API mode, also set `ANALYTICS_API_BASE_URL` to the analytical server used by `VITE_API_BASE_URL`; the bridge retrieves and validates its own facts. `npm run agent` remains a standalone local bridge on loopback port 8787 (`AGENT_PORT` overrides it); the [Render deployment](../../docs/deployment.md) uses the combined production server.
