...
function InvestigationApp({ onUpload }: { onUpload: () => void }) {
  ...
  return <TooltipProvider delayDuration={250}><div className="app-shell canvas-shell" ref={shell}>
    <header className="app-header canvas-header">
      ...
      <Button variant="outline" aria-label="Загрузить CSV" title="Дополнительный обзор CSV с переводами" onClick={onUpload}><Upload data-icon="inline-start" /><span className="header-action-text">CSV переводов</span></Button>
