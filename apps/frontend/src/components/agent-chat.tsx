...
export function AgentChat({ gid, onSelect, active, question, setQuestion, session }: { gid: string | null; onSelect: (gid: string) => void; active: boolean; question: string; setQuestion: (value: string) => void; session: AgentSession }) {
  ...
  return <section className="agent-chat" aria-label="AI-ассистент" hidden={!active}>
    <div className="chat-heading"><div><h2><Bot className="size-4" />Помощник аналитика</h2><p>Объясняет факты и предлагает следующий шаг</p></div><Button variant="ghost" size="icon-sm" aria-label="Новый диалог" disabled={pending} onClick={reset}><RotateCcw /></Button></div>
    <div className="chat-context"><span>Контекст следующего вопроса</span><strong>{gid || 'Список приоритетов'}</strong>{isDemo && <Badge variant="secondary">Демо-данные</Badge>}</div>
    ...
    <form className="chat-composer" onSubmit={submit}><Field><FieldLabel htmlFor="agent-question" className="sr-only">Вопрос аналитику AI</FieldLabel><Textarea ref={input} id="agent-question" placeholder="Задайте вопрос о клиенте или сети…" value={question} maxLength={2000} disabled={pending || !available} onChange={e => setQuestion(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing) { e.preventDefault(); void send(question.trim()) } }} /></Field><div className="chat-composer-actions"><Select value={review} onValueChange={setReview} disabled={pending || !status.data?.reviewerAvailable}><SelectTrigger size="sm" aria-label="Проверка выводов NVIDIA"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="off">OpenAI</SelectItem><SelectItem value="on">OpenAI + проверка NVIDIA</SelectItem></SelectGroup></SelectContent></Select>{pending ? <Button type="button" variant="outline" size="icon" aria-label="Остановить ответ" onClick={cancel}><Square /></Button> : <Button type="submit" size="icon" aria-label="Отправить вопрос" disabled={!question.trim() || !available}><ArrowUp /></Button>}</div><p>AI помогает проверить гипотезу. Сверяйте выводы с числами.</p></form>
  </section>
}
