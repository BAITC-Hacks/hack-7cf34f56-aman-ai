...
function NextRequest({ node }: { node: NodeCard }) {
  return <Card size="sm"><CardHeader><CardTitle>Следующий шаг</CardTitle></CardHeader><CardContent><p className="evidence-text">{node.next_action ?? 'Рекомендация не передана в этот обзор данных.'}</p></CardContent>{node.next_action && <CardFooter><span className="subtle">Запрос дополнительных данных</span><CopyButton value={node.next_action} label="Копировать рекомендацию" /></CardFooter>}</Card>
}
