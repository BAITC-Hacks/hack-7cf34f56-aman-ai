...
export function NodeInspector({ gid, node, loading, error, retry, onSelect, onCluster, onExplain }: Props) {
  ...
  return <aside className="inspector panel" aria-labelledby="inspector-heading">
    ...
    {!gid ? <EmptyPanel title="Здесь начинается проверка" description="Выберите клиента в списке или на графе, чтобы изучить его связи и признаки роли." /> : loading ? <LoadingPanel /> : error ? <ErrorPanel error={error} retry={retry} /> : node ? <>
      <div className="node-identity"><div className="eyebrow">КЛИЕНТ · ГЛУБИНА {node.depth}</div><div className="flex items-center justify-between gap-1"><h3 className="node-gid">{node.gid}</h3><CopyButton value={node.gid} /></div><div className="flex flex-wrap items-center gap-2"><RoleBadge role={node.role} /><Badge variant={node.is_seed ? 'secondary' : 'outline'}>{node.is_seed ? 'Исходный клиент' : 'Не seed'}</Badge><Button variant="ghost" size="xs" onClick={() => onCluster(node.cluster_id)}><Layers data-icon="inline-start" />Кластер {node.cluster_id}</Button></div></div>
      <Tabs key={node.gid} defaultValue="summary" className="min-h-0 flex-1 gap-0"><TabsList variant="line" className="mx-4 mb-3 w-[calc(100%-2rem)]"><TabsTrigger value="summary">Обзор</TabsTrigger><TabsTrigger value="evidence">Основания</TabsTrigger><TabsTrigger value="connections">Связи</TabsTrigger></TabsList>
        <ScrollArea className="min-h-0 flex-1"><div className="inspector-content">
          ...
          <TabsContent value="summary"><div className="flex flex-col gap-4">
            ...
            <Separator /><div className="flex items-center justify-between gap-2"><span className="flex items-center gap-1 text-sm">Исходные клиенты в цепочке<Hint text="Число разных seed, от которых существует наблюдаемый направленный путь. Не доказывает движение одних и тех же средств." /></span><strong>{node.seed_reach_count ?? 'Нет данных'}</strong></div>
            <NextRequest node={node} /><Button variant="outline" onClick={onExplain}><Sparkles data-icon="inline-start" />Объяснить с AI</Button>
          </div></TabsContent>
          <TabsContent value="evidence"><div className="flex flex-col gap-4"><p className="subtle">Оценки — признаки структуры, а не вероятность виновности.</p><h4 className="section-label">Оценки ролей</h4>{roles.map(role => { const detail = node.role_scores[role]; return <div key={role} className="role-score"><div className="flex items-center justify-between gap-2"><span>{roleLabels[role]}</span><span>{!detail.applicable ? 'Не применяется' : detail.score === null ? 'Нет данных' : percent(detail.score)}</span></div>{detail.applicable && detail.score !== null ? <Progress value={detail.score * 100} aria-label={roleLabels[role]} /> : <p className="subtle">{detail.reason}</p>}</div> })}<Separator /><h4 className="section-label">Из чего складывается приоритет</h4>{node.priority_components.map(c => <div className="contribution" key={c.key}><span>{c.key}<small>Вес {percent(c.weight)}</small></span><strong>{c.computed ? `+${(c.contribution * 100).toFixed(1)}` : 'Не рассчитано'}</strong></div>)}{limitations.filter(l => l.code === 'synthetic').map(l => <p className="subtle" key={l.code}>{l.message}</p>)}</div></TabsContent>
