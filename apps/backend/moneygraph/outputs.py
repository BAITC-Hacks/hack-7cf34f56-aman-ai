...
def write_outputs(features,clusters,output_dir):
 ...
 for c in clusters.itertuples(index=False):
  members=features[features.cluster_id==c.cluster_id].sort_values(["priority_score","gid"],ascending=[False,True],kind="stable")
 ...
 ranked=features.sort_values(["priority_score","gid"],ascending=[False,True],kind="stable").head(20).copy()
