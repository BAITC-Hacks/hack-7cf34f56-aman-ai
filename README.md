...
**Проверено при подготовке README:** два запуска пайплайна дали одинаковые CSV и заняли около 1,5 секунды каждый в текущем окружении; прошли 43 теста Vitest, 16 тестов экспортёра, lint и production-сборка. Это локальное измерение, не гарантия времени на другой машине. pytest не удалось запустить: он отсутствовал в окружении, а установка была недоступна из-за ошибки DNS к PyPI. Отдельный скрипт `test:agent` завершается ошибкой из-за запуска Vitest-тестов через `node --test`; серверные тесты проходят в составе `npm test`. Браузерные проверки и живые запросы к AI в рамках подготовки README не запускались.
