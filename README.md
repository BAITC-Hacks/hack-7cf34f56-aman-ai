...
# Добавьте локальные ключи, затем загрузите их только в процесс:
set -a; source .env.local; set +a
.venv/bin/python apps/backend/investigate.py "Почему узел 100000006638021100 важен?" --context-gid 100000006638021100
```

При запущенном FastAPI тот же сценарий доступен через `POST /api/investigate` с телом `{"question":"...","context_gid":"100000006638021100"}`; `context_gid` опционален. Реализованы восемь ограниченных инструментов: `get_node`, `get_cluster`, `find_common_downstream`, `get_paths`, `get_seed_paths`, `get_temporal_patterns`, `compare_nodes` и `get_methodology_context`. Запрос ограничен 2 000 символами, выполнение — шестью раундами модели и двенадцатью вызовами инструментов. Ответ содержит объяснение, структурированные evidence/limitations и журнал полученных фактов; пути и выборки ограничены и сопровождаются предупреждениями о полноте. Модель не изменяет роли, приоритеты или файлы результатов.

Единый семантический словарь и локальный валидатор не позволяют трактовать degree как число транзакций, процентиль как фактическую долю или обычную глубину как границу наблюдения. Допускается не более одной коррекции ответа, затем используется осторожный детерминированный fallback. NVIDIA может опционально проверить ответ; его сбой не блокирует уже grounded OpenAI-ответ и отражается в `critic.status`.

Без SDK или настройки OpenAI этот маршрут возвращает 503; детерминированный API и `main.py` продолжают работать. Ключи хранятся только в игнорируемом `.env.local` и не возвращаются клиенту. Подробности: [контракт и проверенный объём AI](docs/backend/ai-investigator.md) и [handoff чата](docs/backend/ai-chat-handoff.md).
